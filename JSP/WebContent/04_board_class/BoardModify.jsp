...
<%
	...
	int result=ModifyArticleService.getInstance().update(board);
...
<%// 3.게시글 수정이 성공적으로 되었다면 그 해당 게시글을 보여주는 페이지로 이동하고
  // 그렇지 않다면, "암호가 잘못 입력되었습니다"를 출력
    if(result==0){%>
    	암호가 잘못 입력되었습니다.
    <%}else{
     response.sendRedirect("BoardView.jsp?id="+board.getArticleId());
     }%>
