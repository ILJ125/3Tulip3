...
// 1.Service에 getArticleList()함수를 호출하여 전체 메세지 레코드 검색 
 List <BoardRec> mList =  null; 
 ListArticleService l = ListArticleService.getInstance();
 mList=l.getArticleList(pNum);
 //전체 페이지 수
 int totalPageCount = l.getTotalCount();
%>
	...
	<% } else { %>
		...
		<tr>
			...
			<%for(int j=0;j<rec.getLevel();j++){ %>
				&nbsp;
			<%}//end of for %>
			<% if(rec.getLevel() !=0){//getLevel==0이면 reply가 아니다 %>
				<img src='imgs/board_re.gif'>
			<%}//end of if %>
			<a href="BoardView.jsp?id=<%=rec.getArticleId()%>"><%=rec.getTitle() %></a></td>
