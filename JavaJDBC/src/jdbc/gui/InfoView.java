...
class InfoView implements ActionListener {
	...
	// ########################################
	// 1. business logic를 실행하는 클래스 변수 선언 //모델
	InfoModel model;
	...
	void selectBytel() {
		//1.사용자가 입력한 전화번호 값을 얻어 오기
		String tel=tf_tel.getText();
		//2.Model에 selectBytel()함수의 인자로 1번 값  보냄
		
		try {
			//3.2번 객체의 각각의 값을 텍스트 필드에 지정
	...
	void showRecord() {
		...
		try {
			...
			for(int i=0;i<list.size();i++) {
				// 2. 리턴받은 ArrayList에서 Record 객체를 하나씩 얻어온후
	...
	void deleteRecord() {
		...
		try {
			model.delete(tel);
			ta.setText("Tel : "+tel+"일 때 레코드가 삭제 되었습니다.");
		} catch (SQLException e) {
			ta.setText("입력실패 : " + e.getMessage());
